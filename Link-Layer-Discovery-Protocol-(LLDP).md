...
|System Description TLV | OS system description in format: \<OS release name\> \<kernel name\> \<kernel release\> \<kernel version\> \<machine hardware name\>. <br\> Can be obtained via Linux tools like:   `sed -ne 's/PRETTY_NAME="\(.*\)"/\1/p' /etc/os-release; uname -s; uname -r; uname -v; uname -m`|
